...
$KW.Utils = {
    ...
    getBaseHtml: function(widgetModel, context, type, accessObj, rowIndex) {
        ...
        var widgetTabId = widgetModel.tabId || "";
        var tabID = context.tabID || "";
        var id = type ? widgetModel.id : (widgetModel.pf + (widgetTabId && ("_" + widgetTabId)) + "_" + widgetModel.id);
        ...
        if(widgetModel.wType != 'Calendar') {
            accessAttr = $KU.getAccessibilityValues(widgetModel);
        }
        var hoverString = ""
        if(widgetModel.onhover) {
            hoverString = "onmouseenter= '$KI.HoverEvent.executeHoverEvent(event, this)' onmousemove= '$KI.HoverEvent.executeHoverEvent(event, this)' onmouseout= '$KI.HoverEvent.executeHoverEvent(event, this)'";
        }
        ...
        + (tabID && " ktabid='" + tabID + "'") + (containerID && " kcontainerID = '" + containerID + "'")
        + (appName && " appname = '" + appName + "'")
        + (contextmenu && " kcontextmenuid = '" + contextmenu.id + "'") + disable
        + (toolTip ? " title= '" + toolTip + "'" : "") + hoverString + " " + kuid + " ";
    },
            ...
            for(var i = 0; i < wArray.length; i++) {
                ...
                switch(wType) {
                    ...
                    case "TabPane":
                        ...
                        $KW.touch.computeSnapWidths(tabPane.id, "Tabpane");
                    ...
                    case "ListBox":
                        if(widgetModel.viewtype == constants.LISTBOX_VIEW_TYPE_EDITVIEW) {
                            ...
                            $KW.ListBox.initializeEditableList(listNode);
                        }
                        break;
                }
    ...
    updateChildModel: function(childModel, data) {
        ...
        if(data instanceof Object) {
            var isGroupWidget = (childModel.wType == "RadioButtonGroup" || childModel.wType == "CheckBoxGroup" || childModel.wType == "ListBox");
            if(isGroupWidget) {
                childModel.masterdata = "";
                childModel.masterdatamap = "";
                if(childModel.wType == "RadioButtonGroup") {
                    childModel.selectedkey = null;
                    !IndexJL && (data.selectedkey = data.selectedKey);
                }
                if(childModel.wType == "CheckBoxGroup" || childModel.wType == "ListBox") {
            ...
            if(data instanceof Array) {
                ...
                for(var prop in data) {
                    ...
                    if(isGroupWidget && data.masterData && data.masterData.length > IndexJL && (key == 'selectedkeys' || key == 'selectedkey')) {
                        $KW.Utils.setSelectedValueProperty(data, data.masterData, key, data[prop]);
                    }
                }
            }
    ...
    setSelectedValueProperty: function(widgetModel, choices, property, value) {
        ...
        switch(property) {
            ...
            case "selectedkey":
                ...
                for(var i = IndexJL; i < (choices.length); i++) {
                    ...
                    if(selectedKey == key) {
                        ...
                        if(value) {
                            widgetModel.selectedKeyValue = widgetModel.selectedkeyvalue;
                        }
                        break;
                    }
                }
            ...
            case "selectedkeys":
                ...
                if(value) {
                    widgetModel["selectedKeyValues"] = widgetModel["selectedkeyvalues"];
                }
                break;
        }
    },
    ...
    scrollTo: function(y, popup) {
        if(popup && popup != "undefined") {
            popup = $KU.getElementById(popup);
            popup.scrollTop = y;
        } else
            window.scrollTo(0, y);
    },

    checkViewport: function(ele) {
        var eleDimensions, parentDimensions, parentNode;
        var html = document.documentElement;
        eleDimensions = ele.getBoundingClientRect();
        parentNode = ele.parentNode;
        parentDimensions = parentNode.getBoundingClientRect();
        while(parentNode.tagName != "FORM") {
            if(((eleDimensions.top >= parentDimensions.top) && (eleDimensions.top <= parentDimensions.bottom)) && ((eleDimensions.right >= parentDimensions.left) && (eleDimensions.left >= parentDimensions.left) && (eleDimensions.left <= parentDimensions.right))) {
                return true;
            }
    ...
    checkElementViewportHeight: function(el) {
        var parentDimensions, checkdiff, formele;
        var parent = el.parentNode;
        var eleDimensions = el.getBoundingClientRect();
        var formchk = false;
        while(parent && !formchk) {
            formele = parent.tagName;
            if(formele == "FORM") {
                formchk = true;
            }
            parentDimensions = parent.getBoundingClientRect();
            diff_top = eleDimensions.bottom - parentDimensions.top;
            diff_bottom = parentDimensions.bottom - eleDimensions.top;
            diff_left = eleDimensions.right - parentDimensions.left;
            diff_right = parentDimensions.right - eleDimensions.left;

            checkdiff = (diff_bottom >= eleDimensions.height / 2) && (diff_top >= eleDimensions.height / 2) && (diff_left >= eleDimensions.width / 2) && (diff_right >= eleDimensions.width / 2);

            if(formchk) {
                if(Math.round(diff_top) >= parentDimensions.bottom) {
                    return false;
                }
            }
    ...
    scrollToElement: function(widgetModel, el) {
        var startY, stopY = 0;
        var popup = false;
        var scrollElement = el;
        var focusElement = el;
        var parentModel = $KU.getFormModel(widgetModel.pf);
        if(parentModel && parentModel.wType == "Popup") {
            popup = true;
        }
        while(scrollElement) {
            if(Math.abs(scrollElement.offsetHeight - scrollElement.scrollHeight) > 1 || (scrollElement.tagName == "HTML" && !popup)) {
                var boxId = scrollElement.id;
                startY = scrollElement.scrollTop;
                stopY = startY + stopY + focusElement.getBoundingClientRect().top - scrollElement.getBoundingClientRect().top;
                var scrollTop = Math.abs(startY);
                var scrollHeight = (scrollElement.tagName == 'BODY') ? $KU.getWindowHeight() : scrollElement.offsetHeight;
                var scrollBottom = scrollTop + scrollHeight;
                var tempHeight = focusElement.offsetHeight;
                var tempTop = focusElement.offsetTop - scrollElement.getBoundingClientRect().top;
                var tempBottom = tempTop + tempHeight;
                if(stopY > scrollHeight || scrollBottom < tempBottom || (scrollTop > 0 && scrollTop > tempBottom) || (scrollElement.tagName == "HTML")) {
                    this.animateY(startY, stopY, boxId);
                    stopY = 0;
                }

                focusElement = scrollElement;
            }
    ...
    animateY: function(startY, stopY, popupID) {

        var distance = stopY > startY ? stopY - startY : startY - stopY;
        if(distance < 100) {
            $KW.Utils.scrollTo(stopY, popupID);
            try {
                el.focus();
            } catch(e) {}
            return;
        ...
        if(stopY > startY) {
            for(var i = startY; i < stopY; i += step) {
                setTimeout("$KW.Utils.scrollTo(" + leapY + ", '" + popupID + "')", timer * speed);
                leapY += step;
                if(leapY > stopY)
                    leapY = stopY;
                timer++;
    ...
    getWidgetNodeFromNodeByModel: function(model, node) {
        if(node) {
            ...
            if(model.wType == 'DataGrid' && ((model.enablescrollbar == 1 && !model.dockingheader) || $KW.FlexUtils.isFlexWidget(model))) {
                map.DataGrid = 1;
            }
    ...
    removeSensitiveText: function(formId) {
        if(formId) {
            var formNode = $KU.getElementById(formId + "_container");
