...
voltmx.appinit = {
    ...
    vendor: (/trident/gi).test(navigator.userAgent) ? 'ms' : ((/webkit/i).test(navigator.userAgent) && !(/edge/i).test(navigator.userAgent)) ? 'webkit' : (/firefox/i).test(navigator.userAgent) ? (Number(navigator.userAgent.match(/Firefox\/(\d+)/)[1]) >= 126 ? '' : 'Moz') : 'opera' in window ? 'o' : ((/msie/i).test(navigator.userAgent) || (/rv:([1][1-9])/i).test(navigator.userAgent)) ? 'ms' : '',
    ...
    transition: ((/edge/gi).test(navigator.userAgent)) || ((/firefox/i).test(navigator.userAgent) && Number(navigator.userAgent.match(/Firefox\/(\d+)/)[1]) >= 126) ? "transition" : "Transition",
    ...
    verifyhref: function(skiploadappjs) {
        ...
        voltmx.appinit.useTransition = (typeof(document.body || document.documentElement).style[voltmx.appinit.vendor + voltmx.appinit.transition] == "string");
    ...
    loadappjs: function(cb) {
        voltmx.appinit.loadScript("jslib/voltmxlicense.js",function(){
...
$KI.os = {
    platform: function() {
        ...
        params["hascamera"] = voltmx.appinit.isCameraSupported;
        ...
        params["osName"] = platform.deviceOSType;
        params["osVersion"] = platform.deviceOSVersion;
