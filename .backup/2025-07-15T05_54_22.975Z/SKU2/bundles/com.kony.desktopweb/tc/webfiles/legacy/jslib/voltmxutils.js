...
function ieSetProperty(obj, key, value) {
    voltmx.model.update(obj, key, value);
}

function ieGetProperty(obj, key) {
    if(!obj) return null;
    if(obj.wType) {
        key = (obj[key] instanceof Object && obj[key].wType) ? key : key.toLowerCase();
    }
    return obj[key];
}
...
var transitionVendor = ((((/firefox/i).test(navigator.userAgent) && Number(navigator.userAgent.match(/Firefox\/(\d+)/)[1]) >= 126) ? '' : undefined) === '') ? '' : vendor;
...
$KU = voltmx.utils = {
    ...
    getFormModel: function(id, app) {
        ...
        } else if ($KG["__currentForm"] && $KG["__currentForm"][id]) {
    ...
    setDisabledProperty: function(node, voltmxAttribute, htmlAttribute, value, widgetModel) {
        ...
        if((widgettype == "RadioButtonGroup" || widgettype == "CheckBoxGroup") && node.tagName == "INPUT" && widgetModel.itemorientation == "vertical") {
            node.parentNode[attributeFunction](voltmxAttribute, value);
        }

        if(widgetModel && widgetModel.wType == "Label") {
            propertyValue = !!(!value && widgetModel.textCopyable) ? "text" : "none";
    ...
    getPosition: function(el, popupModel) {
        var left = 0,
            top = 0,
        ...
        if(!(popupModel && popupModel.ismodal)) {
    ...
    getAnchorPosition: function(widgetModel, anchorElement, contextWidgetNode) {
        var widgetContext = widgetModel.context;
        var leftPos = 0,
            topPos = 0;
        var bodyWidth = document.getElementsByTagName("body")[0].offsetWidth;
        var bodyHeight = document.getElementsByTagName("body")[0].offsetHeight;
        var bodyAlign = document.getElementsByTagName("body")[0].getAttribute("bodyalign");
        var screenDelta = screen.width - bodyWidth;
        var contextElementPosition = 0;
        if(contextWidgetNode) {
            contextElementPosition = $KU.getPosition(contextWidgetNode);
        } else {
            if(widgetContext.widget.wType == 'Form') {
                contextElementPosition = $KU.getPosition(document.getElementById(widgetContext.widget.id));
            } else {
                contextElementPosition = $KU.getPosition($KU.getNodeByModel(widgetContext.widget, widgetModel));
            }
        }
        ...
        if(widgetContext.anchor == "bottom") {
            if((contextElementPosition.bottom + anchorElement.offsetHeight > bodyHeight) && (widgetModel.wType != "Calendar"))
        ...
        if(widgetContext.anchor == "left") {
            ...
            if(bodyAlign == "center") {
                if(contextElementPosition.left - anchorElement.offsetWidth < screenDelta / 2) {
                    leftPos = screenDelta / 2;
                } else {
                    leftPos = contextElementPosition.left - anchorElement.offsetWidth;
                }
            }
            ...
            if(bodyAlign == "right") {
                if(contextElementPosition.left - anchorElement.offsetWidth < screenDelta) {
                    leftPos = 0;
                } else {
                    leftPos = contextElementPosition.left - anchorElement.offsetWidth - screenDelta;
                }
            }
        }
        if(widgetContext.anchor == "right") {
            ...
            if(bodyAlign == "center") {
                if(contextElementPosition.left + contextElementPosition.width + anchorElement.offsetWidth > bodyWidth + screenDelta / 2) {
                    leftPos = contextElementPosition.left + contextElementPosition.width - (contextElementPosition.left + contextElementPosition.width + anchorElement.offsetWidth - bodyWidth);
                    leftPos = leftPos + screenDelta / 2;
                } else {
                    leftPos = contextElementPosition.left + contextElementPosition.width;
                }
            }
            if(bodyAlign == "right") {
                ...
                if(contextElementPosition.left + contextElementPosition.width + anchorElement.offsetWidth > bodyWidth + screenDelta) {
                    leftPos = contextElementPosition.left + contextElementPosition.width - (contextElementPosition.left + contextElementPosition.width + anchorElement.offsetWidth - bodyWidth);
                } else {
                    leftPos = contextElementPosition.left + contextElementPosition.width - screenDelta;
                }
            }
        }
    ...
    getNodeByModel: function(model) {
        if(model) {
            if(model.isCloned == true && model.kuid) {
            ...
            if(widgetTabId)
                return document.getElementById(model.pf + "_" + widgetTabId + "_" + model.id);
            else
                return document.getElementById(model.pf + "_" + model.id) || document.getElementById(model.id);
        } else {
            return null;
        }
    },

    getElementID: function(id, isheader) {
        return(id && typeof id == "string") ? (isheader ? id.substring(id.lastIndexOf("_") + 1, id.length) : id.substring(id.indexOf("_") + 1, id.length)) : null;
    },
    ...
    getModelByNode: function(target) {
        ...
        var widgetID = (target && target.getAttribute("id") && target.getAttribute("id").split("_").length == 3) ? this.getElementID(target.getAttribute("id"), true) : this.getElementID(target.getAttribute("id"));

        if(target.getAttribute('kwidgettype') == "CalendarText") {
            widgetID = target.id.split("_")[0];
        }
        var tabPaneID = target.getAttribute("ktabpaneid");
        var formID = target.getAttribute("kformname");
        var tabID = target.getAttribute("ktabid")
        return voltmx.model.getWidgetModel(formID, widgetID, tabPaneID, tabID);
    },
    ...
    getPlatform: function() {
        if(!$KU.platform) {
            ...
            platform.deviceOSType = $KU.getPlatformDeviceOS();
            platform.deviceOSVersion = $KU.getPlatformDeviceOSVersion(platform.deviceOSType);
    ...
    getPlatformName: function() {
        if(voltmx.appinit.isIE)
            return "IE";
        if(voltmx.appinit.isFirefox)
            return "Firefox";
        if(voltmx.appinit.isChrome)
            return "Chrome";
        if(voltmx.appinit.isSafari)
            return "Safari";
        if(voltmx.appinit.isOpera)
            return "Opera";
        if(voltmx.appinit.isEdge)
            return "Edge";
        else
            return "";
    },
    ...
    getPlatformVersion: function(name) {
        ...
        switch(name) {
            case "IE":
                userAgent.match(/MSIE ([0-9]+)/) || userAgent.match(/rv:(\d+)/);
                ver = RegExp.$1;
                break;
            case "Firefox":
                userAgent.match(/Firefox[\/\s](\d+\.\d+)/);
                ver = RegExp.$1;
                break;
            case "Chrome":
    ...
    getPlatformDeviceOS: function() {
        if(navigator.userAgent.indexOf('Android') >= 0) {
            return constants.DEVICE_OSNAME_ANDROID;
        }
        if(navigator.userAgent.indexOf('Android') >= 0 && !(navigator.userAgent.indexOf('mobile') >= 0)) {
            return constants.DEVICE_OSNAME_ANDROIDTABLET;
        }
        if(navigator.userAgent.indexOf('iPhone') >= 0) {
            return constants.DEVICE_OSNAME_IPHONE;
        }
        if(navigator.userAgent.indexOf('iPad') >= 0) {
            return constants.DEVICE_OSNAME_IPAD;
        }
        if(navigator.userAgent.indexOf('Macintosh') >= 0) {
            return constants.DEVICE_OSNAME_MACINTOSH;
        }
        if(navigator.userAgent.indexOf('Windows NT') >= 0 && !voltmx.appinit.isWindowsTablet) {
            return constants.DEVICE_OSNAME_WINDOWS;
        } else if (voltmx.appinit.isWindowsTablet) {
            return constants.DEVICE_OSNAME_WINDOWSTABLET;
        }
        if(navigator.userAgent.indexOf('Windows Phone') >=0 && !(navigator.userAgent.indexOf('Android') >= 0)) {
            return constants.DEVICE_OSNAME_WINDOWPHONE;
        }
        if((navigator.userAgent.indexOf('X11') >= 0) || ((navigator.userAgent.indexOf('Linux') >= 0) && !(navigator.userAgent.indexOf('Android') >= 0))) {
            return constants.DEVICE_OSNAME_LINUX;
        }
        if(voltmx.appinit.isBlackBerryNTH) {
            return constants.DEVICE_OSNAME_BLACKBERRYNTH;
        }
        if(voltmx.appinit.isBlackBerry || voltmx.appinit.isPlaybook) {
            return constants.DEVICE_OSNAME_BLACKBERRY;
        }
        return "";
    },

    getPlatformDeviceOSVersion: function(name) {
        var ver = "";
        var winOS = {
            '10.0': '10',
            '6.3': '8.1',
        ...
        switch(name) {
            ...
            case 'windowstablet':
                ...
                break;
            case 'Macintosh':
                userAgent.match(/OS X ([0-9_]+)/);
                ver = RegExp.$1.replace(/_/g, '.');
                break;
            case 'windows':
                userAgent.match(/Windows NT ([0-9.]+)/);
                ver = winOS[RegExp.$1];
                break;
            default:
                ver = 0;
    ...
    setorientationDelay: function() {
        var delay = 100;
        if(voltmx.appinit.isIDevice && voltmx.appinit.isSafari) {
            delay = 0;
        }
        $KU.orientationDelay = delay;
    },
    ...
    imgLoadHandler: function(event, img) {
        ...
        var tabID = img.getAttribute("ktabid");
        var widgetModel;

        if(tabID) {
            widgetModel = voltmx.model.getWidgetModel(img.getAttribute("kformname"), $KU.getElementID(targetWidgetID), tabPaneID, tabID);
        } else {
    ...
    transition: (transitionVendor) ? transitionVendor + "Transition" : "transition",
    transform: (vendor) ? vendor + "Transform" : "transform",
    transitionDelay: (transitionVendor) ? transitionVendor + "TransitionDelay" : "transitionDelay",
    transitionDuration: (transitionVendor) ? transitionVendor + "TransitionDuration" : "transitionDuration",
    transitionProperty: (transitionVendor) ? transitionVendor + "TransitionProperty" : "transitionProperty",
    ...
    setScrollHeight: function(boxModel, scrollBox) {
        ...
        if((parentModel && (parentModel.wType == 'FlexContainer' || parentModel.wType == 'FlexScrollContainer')) || (topLevelModel && topLevelModel.wType == 'Form' && topLevelModel.layouttype != voltmx.flex.VBOX_LAYOUT)) {
            ...
            if(boxModel.wType == 'Segment') scroller.style.overflowX = "hidden";

            if(boxModel.wType == 'Browser') {
                var checkExternalUrl = (boxModel.url) ? $KW.Browser.isExternalUrl(boxModel.url) : true;
                if(!checkExternalUrl) scroller.style.overflowY = "hidden";
            }
            return;
        }
        ...
        if((boxModel.containerheight || boxModel.containerheight == 0) && boxModel.wType != "ScrollBox") {
