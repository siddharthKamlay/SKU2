    ...
    var _createWidgetHierarchy = function UI$_createWidgetHierarchy(model) {
        ...
        $KW.iterate(model, function(widget) {
            ...
            if(this !== parent && $KU.is(this[widget.id], 'widget') && !widget.childHierarchy) {
                ...
                if ((voltmx.application.getCurrentForm() == null || voltmx.application.getCurrentForm().id != currentFormName) && parent instanceof voltmx.ui.FlexContainer) {
                    if (!parent[widget.id]) {
                        parent[widget.id] = widget;
                    }
                    parent.childHierarchy = true;
                    let topLevelModel = parent.parent;
                    while (topLevelModel) {
                        if (topLevelModel && !topLevelModel[widget.id]) {
                            topLevelModel[widget.id] = widget;
                        }
