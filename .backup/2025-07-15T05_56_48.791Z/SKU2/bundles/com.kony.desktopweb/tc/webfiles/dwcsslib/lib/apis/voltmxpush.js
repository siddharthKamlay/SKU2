...
Object.defineProperty(voltmx, 'push', {configurable:false, enumerable:false, writable:false, value:(function() {
    ...
    function __getPermission(config) {
        ...
        Notification.requestPermission().then(function() {
            ...
            if(voltmxSwRegistration) {
                ...
                // Pass the messaging instance and configuration to __getToken for token generation.
                __getToken(messaging, config);
            } else {
    ...
    /*
    Methodname: __getToken
    Description: Method to retrieve the push notification token using the provided VAPID key and service worker registration.
    Parameters: messaging(firebase.messaging.Messaging), config(Object), vapidKey(String) - public key for enabling Web Push Protocol , voltmxSwRegistration(Object) - Registered service worker for push notifications
    Return: None
    */
    function __getToken(messaging, config) {
        messaging.getToken({ vapidKey: config.publicKey, serviceWorkerRegistration: voltmxSwRegistration }).then(function(token) {
            if(token) {
