...
function __createHierarchy(that, widgetref, containerType) {
    ...
    if((voltmx.application.getCurrentForm() == null || voltmx.application.getCurrentForm().id != currentFormName) && that.parent instanceof voltmx.ui.FlexContainer){ 
        that[widgetref.id] = widgetref; 
        widgetref.childHierarchy = true; 
    }
...
function containerWidgetExtendRemoveReferences(widgetref) {
    ...
    var index = chldref.indexOf(widgetref);
    chldref && chldref.splice(index, 1);
...
function removeReferencesFromTopLevelModel(widgetModel, topLevelModel) {
    ...
    if(topLevelModel) {
        var parentTopLevelModel = topLevelModel.parent;
        delete topLevelModel[widgetModel.id];
        while(parentTopLevelModel) {
            if(parentTopLevelModel && parentTopLevelModel[widgetModel.id]) {
                delete parentTopLevelModel[widgetModel.id];
            }
            parentTopLevelModel = parentTopLevelModel.parent;
        }
