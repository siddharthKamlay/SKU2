(function() {
    ...
    var _handleSkinEvent = {
        ...
        dealWithGroupWidgetHoverSkin: function(model) {
            var $K = voltmx.$kwebfw$, $KU = $K.utils, $KW = $K.widget,
                ...
                hStateSkinProperties = model.hoverStateSkinProperties;
            ...
            || ['CheckBoxGroup', 'RadioButtonGroup'].indexOf($KW.name(model)) >= 0) {
                ...
                } else if(model.hoverSkin !== '' || (hStateSkinProperties && typeof hStateSkinProperties === 'object' && !Array.isArray(hStateSkinProperties))) {
        ...
        getPrioritisedSkinType: function(model) {
            var $K = voltmx.$kwebfw$, $KW = $K.widget, skinType = '',
                ...
                fStateSkinProperties = model.focusStateSkinProperties, hStateSkinProperties = model.hoverStateSkinProperties;
            ...
            if(['CheckBoxGroup', 'RadioButtonGroup'].indexOf($KW.name(model)) >= 0) {
                ...
                if((fSkin || (fStateSkinProperties && typeof fStateSkinProperties === 'object' && !Array.isArray(fStateSkinProperties))) && fNode && (currNode === fNode)) {
                    skinType = 'focusSkin';
                } else if((hSkin || (hStateSkinProperties && typeof hStateSkinProperties === 'object' && !Array.isArray(hStateSkinProperties))) && hNode && (currNode === hNode)) {
                    skinType = 'hoverSkin';
                } else if(aSkin && aNode && (currNode === aNode)) {
                    skinType = 'activeStateSkin';
                }
            } else {
                if((fSkin || (fStateSkinProperties && typeof fStateSkinProperties === 'object' && !Array.isArray(fStateSkinProperties))) && fModel && (currModel === fModel)) {
                    skinType = 'focusSkin';
                } else if((hSkin || (hStateSkinProperties && typeof hStateSkinProperties === 'object' && !Array.isArray(hStateSkinProperties))) && hModel && (currModel === hModel)) {
                    skinType = 'hoverSkin';
        ...
        setSkin: function(model, skin, type) {
            ...
            if(el) {
                ...
                && model.viewType === constants.LISTBOX_VIEW_TYPE_EDITVIEW)) {
                    if($KU.is(model, 'widget', 'ListBox')) {
                        if(type === 'hover' && (model.hoverSkin || (model.hoverStateSkinProperties && typeof model.hoverStateSkinProperties === 'object' && !Array.isArray(model.hoverStateSkinProperties)))) {
                            $KD.addCls(el, model.hoverSkin + '-hover');
                        } else if(type === 'focus' && (model.focusSkin || (model.focusStateSkinProperties && typeof model.focusStateSkinProperties === 'object' && !Array.isArray(model.focusStateSkinProperties)))) {
    ...
    var _registerHoverSkinEvent = function $KE_registerHoverSkinEvent(body) {
        ...
        $KD.on(
            ...
            $KU.debounce(_handleSkinEvent.dealWithHoverSkin, 0),
            {passive:false}
        );
    };
